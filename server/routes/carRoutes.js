...
router.post("/register", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to register car", error: err.message });
  }
});
...
router.post("/rent/:id", async (req, res) => {
  ...
  try {
    ...
    const existingRental = await Car.findOne({
      rentedBy: driverId,
      isRented: true,
    });
    if (existingRental) {
      return res
        .status(400)
        .json({ message: "Driver already has an active rental" });
    }
...
router.get("/rented/:driverId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch rented cars", error: err.message });
  }
});
...
router.put("/return/:carId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to return car", error: err.message });
  }
});
...
router.put("/revoke/:carId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to revoke car rental", error: err.message });
  }
});
