...
const MyRentals = () => {
  ...
  useEffect(() => {
    const fetchRentedCars = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/api/cars/rented/${user._id}`);
        const carsData = response.data;

        // Format the rentEndTime (return date) for each car
        const carsWithFormattedDate = carsData.map(car => ({
          ...car,
          rentEndTimeFormatted: car.rentEndTime ? new Date(car.rentEndTime).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) : ''
        }));
  ...
  const handleReturnCar = async (carId) => {
    ...
    try {
      ...
      toast.error('Failed to return the car. Please try again later.', { id: returnToast });
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-10">
      ...
      <div className="flex flex-wrap justify-center">
        {rentedCars.length > 0 ? (
          rentedCars.map((car) => (
            <div key={car._id} className="max-w-sm rounded overflow-hidden shadow-lg bg-white m-4">
              ...
              <div className="px-6 py-4">
                ...
                <p className="text-gray-700 text-base">Number Plate: {car.plateNo}</p>
          ...
          <p className="text-center text-xl">You haven't rented any cars yet.</p>
        )}
      </div>
    </div>
  );
